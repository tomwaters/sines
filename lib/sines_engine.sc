  ...
  alloc {
    ...
    var def = SynthDef.new(\sin, {
      arg out, vol=0.0, hz=220, hz_lag=0.005,
        env_bias=0.0, amp_atk=0.001, amp_rel=0.05, amp_nil=0,
      ...
      amp_ = EnvGen.ar(Env.circle([0, 1, 0, 0], [amp_atk, amp_rel, amp_nil]), levelBias: env_bias);
    ...
    #[\hz, \vol, \env_bias, \pan, \amp_atk, \amp_rel, \amp_nil, \hz_lag, \pan_lag, \fm_index].do({
      arg name;
